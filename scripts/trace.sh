...
write_archive_entry() {
    ...
    # 生成 references YAML（如果有）
    local refs_yaml=""
    if [ -n "${trace_references:-}" ]; then
        refs_yaml="references:"
        for ref in ${trace_references}; do
            refs_yaml="${refs_yaml}"$'\n'"  - path: ${ref}"
        done
    fi
...
main() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --reference) trace_references="${trace_references} $2"; shift 2 ;;
